...
![Forces](_img/locomotive.png){width=50%}
...
Dessa forma, a função anterior fica limita a esses dois limites. Como exemplo, considere a velocidade máxima de 90 km/h e a força tratora máxima pela corrente elétrica, limitada pela **Velocidade Mínima de Operação Constante** (VMOC) de 20 km/h.
...
A tabela abaixo[^fad] descreve valores típicos do *coeficiente de aderência* $f$ para várias condições do trilho.
...
[^fad]: Hay, W. (1982) *Railroad Engineering*. Wiley & Sons, New York, 2ª edição.
...
2. Para este trecho, determine a velocidade de operação ($V_{op}$) a ser adotada, lembrando que não se pode operar o trem abaixo da **VMOC** -- *Velocidade Mínima de Operação Constante*;
...
6. A patir dos valores dos itens (4) e (5), determine o tamanho da composição em termos de: $n_{Loc}$ locomotivas transportanto $n^{vag}$ vagões.
